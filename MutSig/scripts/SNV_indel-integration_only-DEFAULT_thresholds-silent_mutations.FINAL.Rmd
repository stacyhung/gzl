...
write.table(varscan.paired.calls, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_me.all/varScan/varScan-t_v_me_all-snvs_indels-filtered_default-silent.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(calls.me, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_me.all/snvs_indels.t_v_me.DEFAULT_thresholds.silent.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
#*  VARSCAN   *
#**************
varscan.paired.snvs <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_blood.all/varScan/all.paired_samples.varScan.snps.filtered.txt", sep = "\t", header = TRUE, fill = TRUE)
varscan.paired.indels <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_blood.all/varScan/all.paired_samples.varScan.indels.filtered.txt", sep = "\t", header = TRUE, fill = TRUE)
...
# print out the results (these will be VarScan not intersected with any other methods)
write.table(varscan.paired.calls, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_blood.all/varScan/varScan-t_v_blood_all-snvs_indels-filtered_default-silent.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
## ###############################################################################################
## Combine and/or extract datasets needed for downstream analysis (i.e. MutSig)
## ###############################################################################################


```{r}
# combine:
# 1. Silent mutations: optimized thresholds applied to non-validation cohort (i.e. tumor vs. ME pairs) (NB: there were no silent mutations identified in tumor vs. blood pairs)
# 2. Non-silent mutations for full exome cohort
calls.silent <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_me.all/snvs_indels.t_v_me.not_validation_cohort.OPTIMIZED_thresholds.silent.txt", sep = "\t", header = TRUE, fill = TRUE)
calls.nonsilent <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/snvs_indels.30_final.default_and_optimized.txt", sep = "\t", header = TRUE, fill = TRUE)

# combine datasets
calls.all <- rbind(calls.silent, calls.nonsilent)

# remove failure samples 
failures <- c("GZ222T", "GZ229T", "GZ310T")
calls.all <- subset(calls.all, !(calls.all$tumor_id %in% failures))

# write to output
write.table(calls.all, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/snvs_indels.30_final.default_and_optimized.incl_silent.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```
