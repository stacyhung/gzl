...
This script applies the optimized pipeline the final exome pair (case GZ197)
...
NB: After manual inspection, a duplicate variant with two gene annotations was identified (and CTAGE5 alias removed)
...
write.table(varscan.paired.calls, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_me.all/varScan/varScan-t_v_me_GZ197-snvs_indels-filtered_optimized.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# add sample name (since this was not a egrep-combined dataset)
strelka.calls$sample <- "GZ197_TLMD"

# extract only columns of interest
strelka.calls <- strelka.calls[,c("sample", "V1", "V2", "V4", "V5", "type")]
...
write.table(calls.all, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/snv_indel_datasets/t_v_me.all/snvs_indels.t_v_me.GZ197.OPTIMIZED_thresholds.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```
...
for (i in seq_along(unique(calls.all$tumor_id))) {
  ...
  max_y_density <- which.max(density(as.numeric(temp$allele_freq_tumor))$y)
  max_x_intercept <- density(as.numeric(temp$allele_freq_tumor))$x[max_y_density]
  max_y_intercept <- density(as.numeric(temp$allele_freq_tumor))$y[max_y_density]
  ...
  AF.plots[[i]] <- ggplot(temp, aes(as.numeric(allele_freq_tumor))) + 
    ...
    annotate("text", 
             ...
             x = max(as.numeric(temp$allele_freq_tumor)),
