...
# filter out variants that are between 40 and 60% and estimate tumor content based on the resulting VAFs
calls.all <- filter(calls.all, calls.all$allele_freq_tumor < 30 | calls.all$allele_freq_tumor > 60)

# filter out dbSNP variants
calls.all <- calls.all[! grepl("rs", calls.all$external_id), ]
