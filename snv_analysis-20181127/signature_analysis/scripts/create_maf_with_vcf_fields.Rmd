...
This script takes mutational data (generated from SNV/indel analyses, in txt format) and extracts relevant fields required for a VCF file, and places them in MAF format so that we can then apply the MAF2VCF tool (since there are currently no tools that converts txt files into VCF files).

The aim is to generate the same mutation files that are used for signature analysis with maftools, except in a different format (maf with additional columns to enable conversion to vcf). The vcf files are then used as input into another tool for extracting mutational signatures, MutationalPatterns.

## Prepare files for maf to vcf conversion
...
# now take columns for MAF generation that are relevant for conversion to VCF format

# required fields:
#   - read depth in tumor [t_depth]
#   - reference allele depth in tumor [t_ref_count]
#   - variant allele depth in tumor [t_alt_count]
#   - read depth in normal [n_depth]
#   - reference allele depth in normal [n_ref_count]
#   - variant allele depth in normal [n_alt_count]
keeps <- c("gene", "tumor_id", "Variant_Classification", "type", "ref", "alt", 
           "depth4_normal", "depth4_tumor", "var_reads_normal", "var_reads_tumor",
           "gt_tumor", "impact", "fun_class", 
           "transcript_id", "chr", "pos", "type")
mutations <- mutations[keeps]

# extract (and calculate as needed) specific depths of interest from depth4 columns:
mutations <- separate(mutations, depth4_normal, into = c("n_ref_reads_f", "n_ref_reads_r", "n_var_reads_f", "n_var_reads_r"), sep = ",", remove = TRUE, convert = TRUE)
mutations <- separate(mutations, depth4_tumor, into = c("t_ref_reads_f", "t_ref_reads_r", "t_var_reads_f", "t_var_reads_r"), sep = ",", remove = TRUE, convert = TRUE)
mutations$t_depth <- mutations$t_ref_reads_f + mutations$t_ref_reads_r + mutations$t_var_reads_f + mutations$t_var_reads_r
mutations$n_depth <- mutations$n_ref_reads_f + mutations$n_ref_reads_r + mutations$n_var_reads_f + mutations$n_var_reads_r
mutations$t_ref_depth <- mutations$t_ref_reads_f + mutations$t_ref_reads_r
mutations$t_var_depth <- mutations$t_var_reads_f + mutations$t_var_reads_r
mutations$n_ref_depth <- mutations$n_ref_reads_f + mutations$n_ref_reads_r
mutations$n_var_depth <- mutations$n_var_reads_f + mutations$n_var_reads_r
...
write.table(mutations, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/signature_analysis/input/snvs_indels.30_minus_GZ222_and_GZ229.default_and_optimized.incl_silent_and_UTR.for_vcf_conversion.maf", sep = "\t", quote = FALSE, row.names = FALSE)
```
