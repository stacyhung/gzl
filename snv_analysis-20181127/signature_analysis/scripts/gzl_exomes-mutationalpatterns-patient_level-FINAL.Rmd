...
plot_data <- plot_data %>% mutate(
     med_status = case_when(
         mediastinal_involvement == "Primary"  ~ "Primary",
         mediastinal_involvement == "Secondary" | mediastinal_involvement == "Not involved" ~ "Not primary"
     )
)

# combine "45 < age < 60" and "age <= 60"
plot_data <- plot_data %>% mutate(
     age_group = case_when(
         age == "< 45" ~ "<= 45",
         age == "45 < age < 60" | age == ">= 60" ~ "> 45"
     )
)
...
# barplot (using absolute numbers of mutations)
p.absolute.ebv <- ggplot(data = plot_data, 
...
# order patients by mediastinal status, then by signature contribution starting with most recurrent group
plot_data <- plot_data[order(plot_data$EBV_status, 
...
# relative plot
# customized relative plot
p.relative.ebv <- ggplot(data = plot_data, aes(factor(x = tumor_id, levels = tumors.ordered), 
...
# calculate relative contribution per gene (useful for a relative contribution plot)
plot_data <- as.data.frame(plot_data %>% dplyr::mutate(relative_contribution = as.numeric(contribution) / total_contribution))
# now order first by thymic status (primary, vs. non-primary) and then by decreasing total contribution by patient
plot_data <- plot_data[with(plot_data, order(as.character(med_status), -total_contribution)), ]
...
mean_contributions <- unique(mean_contributions)
colnames(mean_contributions) <- c("signature_group", "mean_contribution")
mean_contributions <- mean_contributions[rev(order(mean_contributions$mean_contribution)),]
# merge with full dataframe
plot_data <- merge(plot_data, mean_contributions, by = "signature_group")
# now specify order of signature groups based on mean contribution
sig_groups.order <- (mean_contributions[rev(order(mean_contributions$mean_contribution)),])$signature_group
...
# barplot (using absolute numbers of mutations)
p.absolute.med <- ggplot(data = plot_data, 
             aes(factor(x = tumor_id, levels = tumors.ordered), 
                     y = as.numeric(contribution), 
                     fill = factor(signature_group, levels = sig_groups.order), 
                     color = med_status)) + 
  geom_bar(stat = "identity", width = 0.7, size = 0.8) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5)) +
  scale_colour_manual(name = "Mediastinal status", 
                      values = c("Not primary"="black", "Primary"="darkorange1")) +
  scale_fill_manual(name = "Signature group",
                    values = c("Aging (1)" = "rosybrown1",
                               "Defective DNA mismatch repair (3, 6, 15, 20, 26)" = "mediumpurple1",
                               "POLE activity (10)" = "lightgreen", 
...
# order patients by mediastinal status, then by signature contribution starting with most recurrent group
plot_data <- plot_data[order(plot_data$med_status, 
                             -plot_data$mean_contribution, 
                             plot_data$relative_contribution),]
plot_data$tumor_id <- factor(plot_data$tumor_id)
tumors.ordered <- unique(plot_data$tumor_id)
...
# relative plot
p.relative.med <- ggplot(data = plot_data, 
             aes(factor(x = tumor_id, levels = tumors.ordered), 
                     y = as.numeric(relative_contribution), 
                     fill = factor(signature_group, levels = sig_groups.order), 
                     color = med_status)) + 
...
contingency_table <- array(dim = c(NUM_signatures*2, NUM_EBV_stati))
i <- 1
...
for (signature_name in signatures){
    ...
    # get values for contingency table
    num.EBV_pos_and_sig <- sum(as.numeric((filter(plot_data, 
                                    EBV_status == "Positive" & signature_group == signature_name))$contribution))
    num.EBV_pos_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    EBV_status == "Positive" & signature_group != signature_name))$contribution)) 
    num.EBV_neg_and_sig <- sum(as.numeric((filter(plot_data, 
                                    EBV_status == "Negative" & signature_group == signature_name))$contribution)) 
    num.EBV_neg_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    EBV_status == "Negative" & signature_group != signature_name))$contribution)) 
    
    # chi-square test
    sam <- array(dim = c(2,2))
    sam[1, ] <- c(num.EBV_pos_and_sig, num.EBV_neg_and_sig)
    sam[2, ] <- c(num.EBV_pos_and_not_sig, num.EBV_neg_and_not_sig)
...
colnames(pval_table) <- c("EBV_status")
row.names(pval_table) <- signatures

not.signatures <- paste("Not ", signatures, sep = "")
contingency.row_names <- c(rbind(signatures, not.signatures))

colnames(contingency_table) <- EBV_stati
row.names(contingency_table) <- contingency.row_names
...
```{r}
signatures <- as.vector(unique(plot_data$signature_group))
age_groups <- as.vector(unique(plot_data$age_group))
...
for (signature_name in signatures){
    ...
    num.under_45_and_sig <- sum(as.numeric((filter(plot_data, 
                                    age_group == "<= 45" & signature_group == signature_name))$contribution))
    num.under_45_and_no_sig <- sum(as.numeric((filter(plot_data, 
                                    age_group == "<= 45" & signature_group != signature_name))$contribution)) 
    num.over_45_and_sig <- sum(as.numeric((filter(plot_data, 
                                    age_group == "> 45" & signature_group == signature_name))$contribution)) 
    num.over_45_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    age_group == "> 45" & signature_group != signature_name))$contribution)) 

    # chi-square test
    sam <- array(dim = c(2,2))
    sam[1, ] <- c(num.under_45_and_sig, num.over_45_and_sig)
    sam[2, ] <- c(num.under_45_and_no_sig, num.over_45_and_not_sig)
...
## Create plot to visualize data from contingency table

Create a (faceted) bar plot showing proportions of each category - each row would be a signature, and each side of the bar  would represent a different category (e.g. left = EBV-, right = EBV+).
...
plot_data <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/signature_analysis/analysis/patient_level-excl_GZ310/chisq_tests/proportions_from_contigency-ALL_clinical_groups-vs-signatures.v3.txt", sep = "\t", header = TRUE)

plot_data$signature_group <- plyr::revalue(plot_data$signature_group, 
                            c("Aging (1)" = "Aging", 
                            "Defective DNA mismatch repair (3, 6, 15, 20, 26)" = "DDMR",
                            "Somatic hypermutation (2, 9, 13)" = "SHM",
                            "POLE activity (10)" = "POLE",
                            "Other signatures with known aetiology (4, 7, 11, 22, 24, 29)" = "Other",
                            "Signatures with unknown aetiology" = "Unknown"))
...
feature_categories.order = c("<= 45", "> 45", 
                             "EBV-", "EBV+",
                             "Non-primary", "Primary",
                             "Group 0 or 3", "Group 1 or 2")

# specify order of signatures
signature_groups.order <- c("Aging", "DDMR", "SHM", "POLE", "Other", "Unknown")

p <- ggplot(data=plot_data, aes(x = factor(signature_group, levels = signature_groups.order), 
                                y = proportion,
                                fill = plot_data$feature_category)) +
  geom_bar(stat="identity", position=position_dodge(), width = 0.8) +
  scale_y_continuous(name = "Mutations attributable to signature / \nTotal mutations") +
  xlab("") +
  theme_bw() +
  theme(legend.position="bottom") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
...
p + facet_grid(. ~ feature_name,  scales="fixed")
```

```{r}
# individual plots - one plot for Age, one for EBV status, one for mediastinal involvement, and one for pathology
plot_data.age <- filter(plot_data, plot_data$feature_name == "Age")
plot_data.ebv <- filter(plot_data, plot_data$feature_name == "EBV status")
plot_data.med <- filter(plot_data, plot_data$feature_name == "Mediastinal involvement")
plot_data.path <- filter(plot_data, plot_data$feature_name == "Pathology")

# Age
p.age <- ggplot(data=plot_data.age, aes(x = factor(signature_group, levels = signature_groups.order), 
                                y = proportion,
                                fill = feature_category)) +
  geom_bar(stat="identity", position=position_dodge(), width = 0.8) +
  scale_y_continuous(name = "Proportion of mutations\nattributable to signature") +
  xlab("") +
  theme_bw() +
  theme(legend.position="bottom") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
...
# EBV status
p.ebv <- ggplot(data=plot_data.ebv, aes(x = factor(signature_group, levels = signature_groups.order), 
                                y = proportion,
                                fill = feature_category)) +
  geom_bar(stat="identity", position=position_dodge(), width = 0.8) +
  scale_y_continuous(name = "Proportion of mutations\nattributable to signature") +
...
p.med <- ggplot(data=plot_data.med, aes(x = factor(signature_group, levels = signature_groups.order), 
                                y = proportion,
                                fill = feature_category)) +
  geom_bar(stat="identity", position=position_dodge(), width = 0.8) +
  scale_y_continuous(name = "Proportion of mutations\nattributable to signature") +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
        plot.margin = unit(c(1,1,1,1), "cm")) +
  scale_fill_manual(values = c("black", "#ff7f00"), name = "") ##fdbf6f

# Pathology
p.path <- ggplot(data=plot_data.path, aes(x = factor(signature_group, levels = signature_groups.order), 
                                y = proportion,
                                fill = feature_category)) +
  geom_bar(stat="identity", position=position_dodge(), width = 0.8) +
  scale_y_continuous(name = "Proportion of mutations\nattributable to signature") +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
        plot.margin = unit(c(1,1,1,1), "cm")) +
  scale_fill_manual(values = c("#fb9a99", "#e31a1c"), name = "")
