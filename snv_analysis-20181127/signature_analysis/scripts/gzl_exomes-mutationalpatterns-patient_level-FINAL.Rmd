...
# read in p-values from chi-square tests
pvalues <- read.table("/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/signature_analysis/analysis/patient_level-excl_GZ310/clinical_groups-chi_square-pvalues.v3.txt", sep = "\t", header = TRUE)

# convert p-values from wide to long
#pvalues.long <- gather(pvalues, feature_name, p.signif, EBV.status:Age)
#pvalues.long$feature_name <- plyr::revalue(pvalues.long$feature_name, 
#                            c("EBV.status" = "EBV status", 
#                            "Mediastinal.involvement" = "Mediastinal involvement"))
#write.table(pvalues.long, "/Volumes/shung/projects/gzl_exomes/snv_analysis-20181127/signature_analysis/analysis/patient_level-excl_GZ310/clinical_groups-chi_square-pvalues.long.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
p.ebv <- ggplot(data=plot_data.ebv, aes(x = factor(signature_group, levels = signature_groups.order), 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
        plot.margin = unit(c(1,1,1,1), "cm")) +
  scale_fill_manual(values = c("#b2df8a", "#33a02c"), name = "")
...
p.med <- ggplot(data=plot_data.med, aes(x = factor(signature_group, levels = signature_groups.order), 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
        plot.margin = unit(c(1,1,1,1), "cm")) +
  scale_fill_manual(values = c("#fdbf6f", "#ff7f00"), name = "")
