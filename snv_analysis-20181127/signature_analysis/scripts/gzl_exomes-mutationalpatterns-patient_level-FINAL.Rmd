...
contingency_table <- array(dim = c(NUM_signatures*2, NUM_med_groups*2))
i <- 1
j <- 1

pval_table <- array(dim = c(NUM_signatures, NUM_med_groups))
k <- 1
l <- 1

# for each gene
for (med_group in med_groups) {
  
  # for each signature
  for (signature_name in signatures){
    
    # get values for contingency table
    num.med_group_and_sig <- sum(as.numeric((filter(plot_data, 
                                    med_status == med_group & signature_group == signature_name))$contribution))
    num.not_med_group_and_sig <- sum(as.numeric((filter(plot_data, 
                                    med_status != med_group & signature_group == signature_name))$contribution)) 
    num.med_group_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    med_status == med_group & signature_group != signature_name))$contribution)) 
    num.not_med_group_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    med_status != med_group & signature_group != signature_name))$contribution)) 
    
    # chi-square test
    sam <- array(dim = c(2,2))
    sam[1, ] <- c(num.med_group_and_sig, num.not_med_group_and_sig)
    sam[2, ] <- c(num.med_group_and_not_sig, num.not_med_group_and_not_sig)
...
not.med_groups <- paste("Not ", med_groups, sep = "")
contingency.col_names <- c(rbind(med_groups, not.med_groups))
not.signatures <- paste("Not ", signatures, sep = "")
contingency.row_names <- c(rbind(signatures, not.signatures))
...
contingency_table <- array(dim = c(NUM_signatures*2, NUM_age_groups*2))
i <- 1
j <- 1

pval_table <- array(dim = c(NUM_signatures, NUM_age_groups))
k <- 1
l <- 1
...
for (age_group in age_groups) {
  ...
  for (signature_name in signatures){
    ...
    # get values for contingency table
    num.age_group_and_sig <- sum(as.numeric((filter(plot_data, 
                                    age == age_group & signature_group == signature_name))$contribution))
    num.not_age_group_and_sig <- sum(as.numeric((filter(plot_data, 
                                    age != age_group & signature_group == signature_name))$contribution)) 
    num.age_group_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    age == age_group & signature_group != signature_name))$contribution)) 
    num.not_age_group_and_not_sig <- sum(as.numeric((filter(plot_data, 
                                    age != age_group & signature_group != signature_name))$contribution)) 
    
    # chi-square test
    sam <- array(dim = c(2,2))
    sam[1, ] <- c(num.age_group_and_sig, num.not_age_group_and_sig)
    sam[2, ] <- c(num.age_group_and_not_sig, num.not_age_group_and_not_sig)
...
colnames(pval_table) <- age_groups
row.names(pval_table) <- signatures

not.age_groups <- paste("Not ", age_groups, sep = "")
contingency.col_names <- c(rbind(age_groups, not.age_groups))
not.signatures <- paste("Not ", signatures, sep = "")
contingency.row_names <- c(rbind(signatures, not.signatures))
