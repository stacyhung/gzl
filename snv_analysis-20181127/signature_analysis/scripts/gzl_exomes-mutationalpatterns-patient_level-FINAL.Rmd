...
Idea: Two-sided rotated bar plot (akin to a forest plot) showing proportions of each category - each row would be a signature, and each side of the bar plot would represent a different category (e.g. left = EBV-, right = EBV+)
...
#p + stat_pvalue_manual(data = pvalues, x = "signature_group", y.position = 0.48, label = 'p.signif')
...
#means <- as.data.frame(compare_means(proportion ~ signature_group, data=plot_data, method = "t.test"))
```
